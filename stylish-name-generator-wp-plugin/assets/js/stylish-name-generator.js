    ...
    if (!isGridBuilt) {
      ...
      for (var c = 0; c < categories.length; c++) {
        ...
        for (var i = 0; i < catStyles.length; i++) {
          ...
          html += '<div class="sng-card">' +
            '<div class="sng-card-text sng-dynamic-text" data-template="' + escapeAttr(catStyles[i]) + '" data-index="' + i + '" style="color:' + state.color + ';font-size:' + state.size + 'px' + shadow + '">' +
            escapeHtml(text) + '</div>' +
            '<div class="sng-card-actions">' +
            ...
            '<button class="sng-card-copy sng-dynamic-copy" data-text="' + escapeAttr(text) + '">Copy</button>' +
            '<button class="sng-card-share sng-dynamic-share" data-text="' + escapeAttr(text) + '" title="Share this style">⤴</button>' +
            '</div>' +
            '</div>';
        }
        html += '</div></div>';
      ...
      var textEls = allSections.querySelectorAll(".sng-dynamic-text");
      var copyBtns = allSections.querySelectorAll(".sng-dynamic-copy");
      var shareBtns = allSections.querySelectorAll(".sng-dynamic-share");
      var favBtns = allSections.querySelectorAll(".sng-card-fav");
      ...
      for (var j = 0; j < textEls.length; j++) {
        ...
        if (el.textContent !== text) {
          el.textContent = text;
          copyBtns[j].setAttribute("data-text", text);
          shareBtns[j].setAttribute("data-text", text);
        }
        
        var isFav = favorites.indexOf(template) !== -1;
        if (isFav) {
          favBtns[j].classList.add("sng-fav-active");
          favBtns[j].textContent = "♥";
          favBtns[j].setAttribute("title", "Remove from favorites");
        } else {
          favBtns[j].classList.remove("sng-fav-active");
          favBtns[j].textContent = "♡";
          favBtns[j].setAttribute("title", "Add to favorites");
        }
        
        el.style.color = state.color;
        el.style.fontSize = state.size + "px";
        if (dark) {
          el.style.textShadow = "0 0 6px rgba(255,255,255,0.45), 0 0 14px rgba(255,255,255,0.2)";
        } else {
